...
import hou.config.Bean;
import hou.config.Property;
import hou.config.parse.ConfigManager;
...
public class ClassPathXmlApplicationContext implements BeanFactory {
			...
			for (Property prop : bean.getProperties()) {
				...
				if (ref != null) {
					...
					if (existBean == null) {
						...
						// 这个语句是不是早点写出来好一些？2016年4月25日12:36:35
	...
	public void showDatil() {
		System.out.println("配置文件中的信息：");
		for (Entry<String, Bean> entry : map.entrySet()) {
			System.out.println(entry.getKey() + "," + entry.getValue());
		}
		System.out.println("生成的对象：");
